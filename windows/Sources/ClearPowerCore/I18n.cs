...
// and the macOS port (macos/Sources/ClearPowerCore/I18n.swift). gettext would follow the
// system locale only; this lets the user switch language at runtime.
using System;
...
namespace ClearPower.Core
{
    public static class I18n
    {
        public static readonly Dictionary<string, Dictionary<string, string>> Strings = new Dictionary<string, Dictionary<string, string>>
        {
            ...
            // Windows
            ["calibNeedsBattery"] = "Unplug the charger first: on Windows the whole-machine power can only be measured from the battery.",
            ["winCalibrateSub"] = "Unplug the charger. The screen turns white and brightness is swept for about 90 s while battery power is measured. Keep the machine idle.",
            ["winChargeMissing"] = "Charge control needs the Lenovo Power Manager driver (installed by Windows Update on ThinkPads). Other vendors are not supported yet.",
            ["winChargeMethod"] = "Charge thresholds are written to the embedded controller through Lenovo Power Manager; they persist across reboots and other operating systems.",
            ["estimateNote"] = "≈ on AC: the whole-machine total is estimated (SoC + memory + calibrated baseline); it is measured only on battery.",
            ["powerSaver"] = "Best power efficiency", ["powerBalanced"] = "Balanced", ["powerPerformance"] = "Best performance",
            ["openSettings"] = "Settings…",
            ["winTray"] = "Tray icon",
            ["winTrayText"] = "The icon shows",
            ["winContentSub"] = "While the popover is open, a tiny thumbnail of the screen is sampled every few seconds to track average brightness (needed for OLED). Nothing is stored.",
            ["winChargeOk"] = "Charge control: Lenovo Power Manager",
            // node tooltips
            ...
            ["aboutVersion"] = "ClearPower {v}",
            // Windows
            ["calibNeedsBattery"] = "请先拔掉充电器：Windows 上只有用电池才能测到整机功耗。",
            ["winCalibrateSub"] = "拔掉充电器后，屏幕将变为全白并在约 90 秒内扫描亮度，同时测量电池输出功率，期间请保持机器空闲。",
            ["winChargeMissing"] = "充电控制需要 Lenovo Power Manager 驱动（ThinkPad 通过 Windows 更新自动安装）。暂不支持其他品牌。",
            ["winChargeMethod"] = "充电阈值通过 Lenovo Power Manager 写入嵌入式控制器，重启和切换系统后仍然有效。",
            ["estimateNote"] = "接电时整机功耗为估算值（SoC + 内存 + 校准基线），标 ≈；只有用电池时才是实测。",
            ["powerSaver"] = "最佳能效", ["powerBalanced"] = "平衡", ["powerPerformance"] = "最佳性能",
            ["openSettings"] = "设置…",
            ["winTray"] = "托盘图标",
            ["winTrayText"] = "图标显示",
            ["winContentSub"] = "弹窗打开时，每几秒取一张极小的屏幕缩略图计算平均亮度（OLED 需要）。不保存任何内容。",
            ["winChargeOk"] = "充电控制：Lenovo Power Manager",
            ["tipAdapter"] = "从充电器取的功率",
            ["tipBattery"] = "电池输出的功率",
            ["tipBatchg"] = "充进电池的功率",
            ["tipSystem"] = "整机功耗（实测）",
        ...
        /// <summary>pref: "system" | "en" | "zh-cn". `systemLanguages` are BCP-47 codes, most preferred first.</summary>
        public static string ResolveLanguage(string pref, IEnumerable<string> systemLanguages)
        {
            if (pref == "zh-cn") return "zh_CN";
            if (pref == "en") return "en";
            return systemLanguages.Any(l => l.ToLowerInvariant().StartsWith("zh")) ? "zh_CN" : "en";
        }
        ...
        public static void SetLanguage(string pref, IEnumerable<string>? systemLanguages = null)
        {
            Current = ResolveLanguage(pref, systemLanguages ?? SystemLanguages());
        }

        public static string T(string key, IDictionary<string, object>? vars = null)
        {
            string s = key;
            if (Strings.TryGetValue(Current, out var cur) && cur.TryGetValue(key, out var v)) s = v;
            else if (Strings["en"].TryGetValue(key, out var en)) s = en;
            if (vars != null)
                foreach (var kv in vars)
                    s = s.Replace("{" + kv.Key + "}", Convert.ToString(kv.Value, CultureInfo.InvariantCulture));
            return s;
        }

        public static string T(string key, string k1, object v1) => T(key, new Dictionary<string, object> { [k1] = v1 });
        public static string T(string key, string k1, object v1, string k2, object v2) => T(key, new Dictionary<string, object> { [k1] = v1, [k2] = v2 });

        public static string FmtDuration(double minutes)
        {
            var m = Math.Max(0, (int)Math.Round(minutes, MidpointRounding.AwayFromZero));
            var h = m / 60; var mm = m % 60;
            if (Current == "zh_CN") return h > 0 ? $"{h} 小时 {mm} 分" : $"{mm} 分钟";
            return h > 0 ? $"{h} h {mm} m" : $"{mm} min";
        }

        public static string FmtW(double w, int? digits = null)
        {
            if (double.IsNaN(w) || double.IsInfinity(w) || w < 0) return "–";
            var d = digits ?? (w >= 100 ? 0 : 1);
            return w.ToString("F" + d, CultureInfo.InvariantCulture) + " W";
        }
    }
}
